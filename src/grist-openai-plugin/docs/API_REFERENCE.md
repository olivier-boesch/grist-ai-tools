...
- `uploadAttachments` relaie chaque fichier reçu vers Grist dans un champ multipart nommé `upload` (conforme au schéma `AttachmentUpload` de la spec OpenAPI officielle de Grist), en s'assurant que l'en-tête `Content-Type` de la requête sortante est bien un `multipart/form-data; boundary=...` généré par `requests` et non le `application/json` par défaut de la session (sinon Grist répond `400 failed upload`).
