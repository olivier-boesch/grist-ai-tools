...
- `upload_attachments` envoie chaque fichier dans un champ multipart nommé `upload` (conforme au schéma `AttachmentUpload` de la spec OpenAPI officielle de Grist : `components/schemas/AttachmentUpload`, `gristlabs/grist-help/api/grist.yml`), plusieurs fichiers pouvant être envoyés dans le même appel. Le client force l'en-tête `Content-Type` à être recalculé par `requests` (au lieu d'hériter du `application/json` par défaut de la session) pour obtenir un `multipart/form-data; boundary=...` valide — sans ce correctif, Grist rejetait la requête avec `400 failed upload`.
